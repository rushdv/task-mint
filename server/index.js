...
async function run() {
  try {
    ...
    app.get("/users/top-workers", async (req, res) => {
      const workers = await usersCol.find({ role: "worker" }).sort({ coins: -1 }).limit(6).toArray();
      res.send(workers);
    });
    ...
    app.patch("/users/role/:id", verifyToken, verifyAdmin, async (req, res) => {
      const result = await usersCol.updateOne({ _id: new ObjectId(req.params.id) }, { $set: { role: req.body.role } });
      res.send(result);
    });
    ...
    app.get("/tasks/buyer/stats", verifyToken, verifyBuyer, async (req, res) => {
      const tasks = await tasksCol.find({ buyer_email: req.decoded.email }).toArray();
      const taskCount = tasks.length;
      const pendingWorkers = tasks.reduce((s, t) => s + (t.required_workers || 0), 0);
      const payments = await paymentsCol.find({ buyer_email: req.decoded.email }).toArray();
      const totalPaid = payments.reduce((s, p) => s + p.amount, 0);
      res.send({ taskCount, pendingWorkers, totalPaid });
    });

    app.get("/tasks/buyer", verifyToken, verifyBuyer, async (req, res) => {
      const tasks = await tasksCol.find({ buyer_email: req.decoded.email }).sort({ completion_date: -1 }).toArray();
      res.send(tasks);
    });
    ...
    app.get("/tasks", verifyToken, async (req, res) => {
      const tasks = await tasksCol.find({ required_workers: { $gt: 0 } }).toArray();
      res.send(tasks);
    });
    ...
    app.patch("/tasks/:id", verifyToken, verifyBuyer, async (req, res) => {
      ...
      const result = await tasksCol.updateOne({ _id: new ObjectId(req.params.id) }, { $set: { task_title, task_detail, submission_info } });
      res.send(result);
    });
    ...
    app.delete("/tasks/:id", verifyToken, async (req, res) => {
      ...
      await usersCol.updateOne({ email: task.buyer_email }, { $inc: { coins: task.required_workers * task.payable_amount } });
      const result = await tasksCol.deleteOne({ _id: new ObjectId(req.params.id) });
      res.send(result);
    });

    app.post("/submissions", verifyToken, verifyWorker, async (req, res) => {
      const sub = req.body;
      const result = await submissionsCol.insertOne({ ...sub, status: "pending", current_date: new Date() });
      await notificationsCol.insertOne({ message: `${sub.worker_name} submitted for: ${sub.task_title}`, toEmail: sub.buyer_email, actionRoute: "/dashboard/buyer-home", time: new Date() });
      res.send(result);
    });
    ...
    app.get("/submissions/worker/stats", verifyToken, verifyWorker, async (req, res) => {
      ...
      const pending = await submissionsCol.countDocuments({ worker_email: email, status: "pending" });
      const approved = await submissionsCol.find({ worker_email: email, status: "approved" }).toArray();
    ...
    app.get("/submissions/worker", verifyToken, verifyWorker, async (req, res) => {
      ...
      const email = req.decoded.email;
      const total = await submissionsCol.countDocuments({ worker_email: email });
      const submissions = await submissionsCol.find({ worker_email: email }).skip((page - 1) * limit).limit(limit).toArray();
    ...
    app.get("/submissions/buyer", verifyToken, verifyBuyer, async (req, res) => {
      ...
      const submissions = await submissionsCol.find({ task_id: { $in: taskIds }, status: "pending" }).toArray();
      res.send(submissions);
    });

    app.patch("/submissions/:id/approve", verifyToken, verifyBuyer, async (req, res) => {
      const sub = await submissionsCol.findOne({ _id: new ObjectId(req.params.id) });
      if (!sub) return res.status(404).send({ message: "Not found" });
      await submissionsCol.updateOne({ _id: new ObjectId(req.params.id) }, { $set: { status: "approved" } });
      await usersCol.updateOne({ email: sub.worker_email }, { $inc: { coins: sub.payable_amount } });
      await notificationsCol.insertOne({ message: `You earned ${sub.payable_amount} coins from ${sub.buyer_name} for ${sub.task_title}`, toEmail: sub.worker_email, actionRoute: "/dashboard/worker-home", time: new Date() });
    ...
    app.patch("/submissions/:id/reject", verifyToken, verifyBuyer, async (req, res) => {
      const sub = await submissionsCol.findOne({ _id: new ObjectId(req.params.id) });
      if (!sub) return res.status(404).send({ message: "Not found" });
      await submissionsCol.updateOne({ _id: new ObjectId(req.params.id) }, { $set: { status: "rejected" } });
      await tasksCol.updateOne({ _id: new ObjectId(sub.task_id) }, { $inc: { required_workers: 1 } });
      await notificationsCol.insertOne({ message: `Your submission for ${sub.task_title} was rejected`, toEmail: sub.worker_email, actionRoute: "/dashboard/my-submissions", time: new Date() });
    ...
    app.post("/payments", verifyToken, async (req, res) => {
      const result = await paymentsCol.insertOne({ ...req.body, date: new Date() });
      await usersCol.updateOne({ email: req.decoded.email }, { $inc: { coins: req.body.coins } });
      res.send(result);
    });

    app.get("/payments", verifyToken, async (req, res) => {
      const payments = await paymentsCol.find({ buyer_email: req.decoded.email }).sort({ date: -1 }).toArray();
      res.send(payments);
    });
    ...
    app.post("/withdrawals", verifyToken, verifyWorker, async (req, res) => {
      ...
      const result = await withdrawalsCol.insertOne({ ...req.body, status: "pending", withdraw_date: new Date() });
      res.send(result);
    });

    app.get("/withdrawals/pending", verifyToken, verifyAdmin, async (req, res) => {
      const withdrawals = await withdrawalsCol.find({ status: "pending" }).toArray();
    ...
    app.patch("/withdrawals/:id/approve", verifyToken, verifyAdmin, async (req, res) => {
      ...
      await usersCol.updateOne({ email: w.worker_email }, { $inc: { coins: -w.withdrawal_coin } });
      await notificationsCol.insertOne({ message: `Your withdrawal of $${w.withdrawal_amount} has been approved`, toEmail: w.worker_email, actionRoute: "/dashboard/withdrawals", time: new Date() });
      res.send({ message: "Approved" });
    });

    app.get("/notifications", verifyToken, async (req, res) => {
      const notifications = await notificationsCol.find({ toEmail: req.decoded.email }).sort({ time: -1 }).toArray();
