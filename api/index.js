...
async function run() {
  try {
    ...
    app.delete("/tasks/:id", verifyToken, async (req, res) => {
      ...
      await usersCol.updateOne({ email: task.buyer_email }, { $inc: { coins: refill } });
    ...
    app.post("/submissions", verifyToken, verifyWorker, async (req, res) => {
      ...
      const result = await submissionsCol.insertOne({ ...submission, status: "pending", current_date: new Date() });
    ...
    app.get("/submissions/worker/stats", verifyToken, verifyWorker, async (req, res) => {
      ...
      const totalEarning = approved.reduce((s, sub) => s + sub.payable_amount, 0);
    ...
    app.get("/submissions/worker", verifyToken, verifyWorker, async (req, res) => {
      ...
      const email = req.decoded.email;
      const total = await submissionsCol.countDocuments({ worker_email: email });
      const submissions = await submissionsCol.find({ worker_email: email }).skip(skip).limit(limit).toArray();
    ...
    app.patch("/submissions/:id/approve", verifyToken, verifyBuyer, async (req, res) => {
      ...
      await notificationsCol.insertOne({
        message: `You earned ${sub.payable_amount} coins from ${sub.buyer_name} for completing ${sub.task_title}`,
    ...
    app.post("/payments", verifyToken, async (req, res) => {
      ...
      const result = await paymentsCol.insertOne({ ...payment, date: new Date() });
      await usersCol.updateOne({ email: req.decoded.email }, { $inc: { coins: payment.coins } });
      res.send(result);
    });
